...
The `esphome` directory in this repo is just a staging directory to provide a location for building the modified components.  It is not setup to run directly under the ESPHome runtime; thus the .yaml files are for illustration only.  See the README-dev.md file in each component folder of the main `esphome-external-components` repo for tips on esphome component development.
